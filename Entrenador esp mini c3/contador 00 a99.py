...
# Pulsador en GPIO4 (con resistencia de pull-up interna)
...
# -------- PROGRAMA PRINCIPAL -----------
contador = 0
estado_prev = 1  # el pulsador está en HIGH al no presionar (pull-up)

while True:
    boton_estado = boton.value()
    
    # Detectar flanco descendente (pulsador presionado)
    if boton_estado == 0 and estado_prev == 1:
        contador += 1
        if contador > 99:
            contador = 0
        estado_prev = 0
        time.sleep_ms(50)  # antirrebote simple

    # Resetear estado si se suelta el pulsador
    if boton_estado == 1:
        estado_prev = 1
